...
%--------------------
	 ...
	 fun {TeleportAcc SnakePositions Xtarget Ytarget Resultat}
	    case SnakePositions of  H|T then
	       if H.to == north then {TeleportAcc T Xtarget Ytarget pos(x:Xtarget y:(H.Ytarget.y)-1 to:H.to)|Resultat}
	       elseif H.to == south then {TeleportAcc T Xtarget Ytarget pos(x:Xtarget y:(H.Ytarget.y)+1 to:H.to)|Resultat}
	       elseif H.to == east then {TeleportAcc T Xtarget Ytarget pos(x:(Xtarget)+1 y:H.Ytarget.y to:H.to)|Resultat}
	       else {TeleportAcc T Xtarget Ytarget pos(x:(Xtarget)-1 y:H.Ytarget.y to:H.to)|Resultat}
	       end
	    else Resultat
	    end
	 end
...
%-------------------- NEW
	 ...
	 else {Ajout {DecodeStrategy Y} {Repeat Y X-1}}
	 end
	 else nil
	 end
      end
...
%---------------------------------------- NEW
	 ...
	 [] repeat(L times:X)|T then {Repeat L X}
	 end
      end
